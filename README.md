...
| `settingsfocus.spec.js` | Where focus is after you press something on the settings screen. Every control there disables itself while it works, and disabling a focused control hands focus to the body — so this walks each one by keyboard and fails if focus ends up nowhere. Plus the two guards: a mouse press must not be rewarded with a focus ring, and a reader who tabbed away during a slow request must be left where they chose to be. And a sweep over every button on the screen, so one added later is covered without anybody remembering. |
| `export.spec.js` | The download and the signed-out screen. The file is read back out of the Blob rather than assumed: that it carries the draft the public endpoints would have hidden, the comment left under somebody else's post that no public endpoint can find, nobody else's writing, and none of the feed's vote or shelf state. That it is dated, indented, detaches its anchor and releases its object URL. Then signed out: that the screen renders rather than redirecting, that it asks the server for **nothing** so no auth error is possible, that the parts renumber from one, that the browser half still works, and that signing in and out from there lands somewhere sensible. |
| `notifyprefs.spec.js` | The last three settings. That only the difference from the default is stored, so a reader who has changed nothing has nothing written down. That a switched-off kind stops being counted **and** stops being listed — the two come from one function, and an emptied list says which switch emptied it. That the other kind still comes through, and that with both off nothing is even asked for. The badge switch clearing the icon immediately rather than at the next change, which is the one place a reader cannot otherwise see it work. Then motion: the attribute the stylesheet branches on, applied before the first paint, computing the same durations as the media query — the check that keeps two copies of six declarations in step — and switching off the view transition, which no stylesheet can reach. |
| `reading-settings.spec.js` | The text size and line width on `#/settings`: that they are the *same* value the reader panel on a post sets rather than a default it overrides — asserted in both directions — and that the data panel's Reset removes both keys rather than writing the defaults into them. Then the specimen, which claims in its own sentence to be set the way a post will be: it is measured against a real post's body at all six combinations of size and width and has to match to the pixel. Plus the one interaction the controls have that is worth seeing — the measure is in `ch`, so a narrow column at Large is wider than a narrow column at Small. |
| `theme.spec.js` | The three states, and the three places that must never disagree about them — storage, the radios on `#/settings`, and the toggle in the header. The round trip the toggle alone could not make (System → pinned → System), the toggle moving you off System, and the radio following the toggle, the palette and the data panel's Reset. Then the half that only shows up later: with `page.emulateMedia` standing in for the machine, System follows it when it changes and a pinned theme does not — plus the title bar's colour following with it, and System surviving a reload as System rather than as whatever it looked like at the time. |
| `browserdata.spec.js` | The data panel: every row naming its keys, its size and what it is for; the sign-in row explaining itself instead of offering a button it shouldn't; each control clearing the disk *and* the line above it, because a panel that says "Forgotten" over "47 posts" has convinced nobody. Then the two that are not just a `removeItem` — emptying the shelf clearing the account's copy and **staying** empty across the reload where a mirror-only version would come back, and the theme reset removing the key rather than writing a default into it, which is the difference between following your system again and being pinned to what it happens to say today. Plus: the panel repainting when something changes it from elsewhere, the sweep leaving you signed in, the demo's own database surviving it, and a grep of the source that fails on any `commons.*` key the panel neither lists nor exempts by name. |
| `accountmenu.spec.js` | The header's one door: three controls left on the surface and the other four behind the avatar, every destination the menu opens onto, and the five ways it closes — a row chosen, a route changed from somewhere else entirely, a press outside, Escape, and the trigger pressed a second time, which is the press a hand-rolled menu gets wrong. Then the keyboard on its own: arrows walking the rows and wrapping at both ends, Home and End, Tab leaving rather than trapping, and settings reached without the mouse ever moving. Plus the panel's arithmetic — right edges flush, on screen at 320px, shrinking rather than running off a short one, following the avatar through a resize — the unread dot and count, axe in both themes, and the one that guards the rebuild: a notification poll landing under an open menu and not shutting it. |
...
email hides below 900px rather than 640px. Both fixes still stand, but the pressure they
were relieving is gone: the signed-in row is three controls now rather than six, because
the destinations moved behind an account menu — see
...
The PWA plan that followed them is finished too — the manifest, the service
worker, the install control and the offline notice all shipped, and what it
decided is in [ADR 0007](docs/adr/0007-a-network-first-service-worker.md) and
[ADR 0010](docs/adr/0010-an-install-control-with-a-silent-state.md).

The settings plan after it is finished as well — the account menu, the
three-part screen, the data panel, the theme, the reading controls,
notifications, motion, About, and an export you can take away — and what it
decided is in
[ADR 0011](docs/adr/0011-a-popover-menu-positioned-in-javascript.md),
